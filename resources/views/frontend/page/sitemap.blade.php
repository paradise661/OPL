...
<urlset xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    @foreach ($all_packs as $page)
        <url>
            <loc>{{ route('productssingle', $page->slug) }}</loc>
            <lastmod>{{ $page->created_at->tz('UTC')->toAtomString() }}</lastmod>
            @if ($page->image)
                <image:image>
                    <image:loc>{{ asset(get_media_url($page->image)) }}</image:loc>
    ...
    @endforeach
    @foreach ($all_cats as $page)
        <url>
            <loc>{{ route('productcategorysingle', $page->slug) }}</loc>
            <lastmod>{{ $page->created_at->tz('UTC')->toAtomString() }}</lastmod>
            @if ($page->image)
                <image:image>
                    <image:loc>{{ asset(get_media_url($page->image)) }}</image:loc>
    ...
    @foreach ($all_brands as $page)
        <url>
            <loc>{{ route('brandsingle', $page->slug) }}</loc>
            <lastmod>{{ $page->created_at->tz('UTC')->toAtomString() }}</lastmod>
            @if ($page->image)
                <image:image>
                    <image:loc>{{ asset(get_media_url($page->image)) }}</image:loc>
